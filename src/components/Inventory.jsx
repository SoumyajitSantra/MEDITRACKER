...
const Inventory = ({ isLoggedIn }) => {
  ...
  // ✅ Your original sample data kept as a fallback
  const sampleData = [
    {
      id: "1",
      name: "Paracetamol",
  ...
  // 🔹 Backend fetch added (minimal change)
  useEffect(() => {
    let isMounted = true;
      ...
      try {
        const res = await fetch("http://localhost:8000/api/medicines", {
          headers: {
            ...
            // if you use token auth, it will be attached; remove if not needed
            Authorization: `Bearer ${localStorage.getItem("token") || ""}`,
          },
          // include credentials if your backend expects cookies:
          // credentials: "include",
        });
        ...
        // basic validation: expect an array of medicines
        if (!Array.isArray(data)) {
          console.warn("Expected array from /api/medicines, got:", data);
        ...
        // On any error fall back to sampleData so UI still works
        console.warn("Failed to fetch medicines from backend, using sampleData:", err);
        if (isMounted) {
          setMedicines(sampleData);
          setFilteredMedicines(sampleData);
        }
  ...
  const getExpiryStatus = (expiryDate) => {
    ...
    if (days < 30) return { color: "bg-orange-100 text-orange-800", text: `${days} days` };
    if (days < 90) return { color: "bg-yellow-100 text-yellow-800", text: `${days} days` };
    return { color: "bg-green-100 text-green-800", text: `${days} days` };
  };

  const getStockStatus = (quantity) => {
    if (quantity === 0) return { color: "bg-red-100 text-red-800", text: "Out of Stock" };
    if (quantity < 10) return { color: "bg-orange-100 text-orange-800", text: "Low Stock" };
    if (quantity < 50) return { color: "bg-yellow-100 text-yellow-800", text: "Medium Stock" };
    return { color: "bg-green-100 text-green-800", text: "Good Stock" };
  };

  const categories = ["All", ...new Set(medicines.map((m) => m.category))];
  const totalValue = filteredMedicines.reduce((sum, med) => sum + med.quantity * med.price, 0);
  const expiredMedicines = filteredMedicines.filter((med) => getDaysUntilExpiry(med.expiryDate) < 0);
  const lowStockMedicines = filteredMedicines.filter((med) => med.quantity < 10);
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Complete overview of your medicine inventory</p>
      </div>
                  ...
                  <tr key={medicine.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">Barcode: {medicine.barcode}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">Qty: {medicine.quantity}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">Total: ${(medicine.price * medicine.quantity).toFixed(2)}</div>
                    </td>

                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{medicine.branchId}</div>
